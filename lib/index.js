...
JSONRPC.prototype.handleJSON = function handleJSON(req, data, callback) {
    ...
    if (!(data instanceof Object)) {
        callback({jsonrpc: "2.0", error:{code: errorCode.parseError, message:"Parse error"},
            id: data.hasOwnProperty('id') ? data.id : null});
        return;
    }
    if (!data.method || !data.params) {
        callback({jsonrpc: "2.0", error:{code: errorCode.invalidRequest, message:"Invalid request"},
            id: data.hasOwnProperty('id') ? data.id : null});
        return;
    }
    if (!this.scope[data.method]) {
        callback({jsonrpc: "2.0", error:{code: errorCode.methodNotFound, message:"Requested method does not exist"},
            id: data.hasOwnProperty('id') ? data.id : null});
        return;
    }
